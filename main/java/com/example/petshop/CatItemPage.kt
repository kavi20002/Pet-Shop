...
import com.example.petshop.databinding.ActivityCatItemPageBinding
import com.google.android.material.bottomnavigation.BottomNavigationView

class CatItemPage : AppCompatActivity() {
    private lateinit var backBtn : ImageView
    private lateinit var cartBtn : Button
    private lateinit var favouriteBtn : ImageView
    private lateinit var binding: ActivityCatItemPageBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityCatItemPageBinding.inflate(layoutInflater)
        ...
        if (intent != null){
            ...
            val desc = intent.getIntExtra("desc", R.string.cat1)
            val image = intent.getIntExtra("image", R.drawable.meowmix)

            binding.CatItemTitle.text = name
            binding.CatItemsPrice.text = price
            binding.catDesc.setText(desc)
            binding.CatItemImage.setImageResource(image)
        }
